...
namespace TeahourFmGUI
{
    public class WebObject
    {
        ...
        {
            List<string> lstDownloadFiles = new List<string>();
            for (int i = 1; i < 59; i++)
            {
                lstDownloadFiles.Add(m_strDownloadDirectory + @"\" + "teahour_episode_" + i.ToString() + m_strFileExtension);
            }

            WebClient downloadClient = new WebClient();
            for (int i = 1; i < lstDownloadFiles.Count + 1; i++)
			{
                string str1 = m_strDownloadDirectory + @"\" + "teahour_episode_" + i.ToString() + m_strFileExtension;
                string str2 = str1.Replace("\\", @"\");
                downloadClient.DownloadFile(str2,
                                            m_strDownloadToDirectory + @"\" + "teahour_episode_" + i.ToString() + m_strFileExtension);
			}        
        }
